...
@Slf4j
@SpringBootTest
class AssertNoteApplicationTests {
...
//        log.info("before:{}", "----------------------------------------");
    ...
    public void testDate() {
        ...
        assertThat(DateTime.now().toDate()).isBeforeYear(2022).isAfterYear(2013);
        // 断言时间再指定范围内 不在指定范围内
    ...
    @Test
    public void testMap() {
        Map<String, Object> foo = Maps.newHashMap("D",4);
        foo.put("A", 1);
        foo.put("B", 2);
        foo.put("C", 3);

        // 断言 map 不为空 size
        assertThat(foo).isNotEmpty().hasSize(4);
        // 断言 map 包含元素
